...
Например необходимо вывести все записи блогов, где у тегов в имени содержится 'ро'.

```python
obj = Entry.objects.filter(tags__name__contains='ро')
    print(obj)
"""<QuerySet [<Entry: Здоровое питание: полезные рецепты>, 
<Entry: Секреты успешного похудения>]>
"""
```
Заметьте, как идёт подключение. Мы пытаемся из таблицы Entry через ```filter``` 
обратиться к полю ```tags``` (которое является связанное отношением многое-ко-многому 
(абсолютно так же делается с отношением
один-к-многому)), у ```tags``` содержатся ссылки на таблицу ```Tag``` у
...
Допустим модель ```User``` и ```AuthorProfile```. У ```AuthorProfile``` есть связь ```OneToOneField``` с
```User```, но обратной связи мы не прописываем, а она есть. Можно идти как вперед,
так и назад.
...
```python
obj = Entry.objects.filter(authors__user__user_profile__city__isnull=True)
print(obj)
"""<QuerySet [<Entry: Последние тренды в мире искусственного интеллекта>, 
<Entry: Приготовление собственного хлеба>, <Entry: Оазисы Сахары: красота и опасность>, 
